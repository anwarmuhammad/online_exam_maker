...
Class Qbank_model extends CI_Model
{
  ...
  function question_list($limit,$cid='0',$lid='0'){
	 if($this->input->post('search')){
		 ...
		 $this->db->or_where('cste_qbank.qid',$search);
		 $this->db->or_like('cste_qbank.question',$search);
		 $this->db->or_like('cste_qbank.description',$search);
	 ...
	 if($lid!='0'){
		 ...
		 $this->db->join('cste_category','cste_category.cid=cste_qbank.cid');
	 $this->db->join('cste_level','cste_level.lid=cste_qbank.lid');
	 $this->db->limit($this->config->item('number_of_rows'),$limit);
		$this->db->order_by('cste_qbank.qid','desc');
		$query=$this->db->get('cste_qbank');
 ...
 function get_option($qid){
	 $this->db->where('qid',$qid);
	 $query=$this->db->get('cste_options');
	 return $query->result_array();
 ...
 function remove_question($qid){
	 ...
	 $this->db->where('qid',$qid);
	 if($this->db->delete('cste_qbank')){
		  $this->db->where('qid',$qid);
			$this->db->delete('cste_options');
		 return true;
	 }else{
 ...
 function insert_question_1(){
	 ...
	 $this->db->insert('cste_qbank',$userdata);
	...
	$userdata=array(
	 ...
	 $this->db->insert('cste_options',$userdata);	 
 ...
 function insert_question_5(){
  ...
  function update_question_1($qid){
	 ...
	 $this->db->where('qid',$qid);
	 $this->db->update('cste_qbank',$userdata);
	 $this->db->where('qid',$qid);
	$this->db->delete('cste_options');
	 foreach($this->input->post('option') as $key => $val){
 ...
 function category_list(){
	 ...
	 $query=$this->db->get('cste_category');
	 return $query->result_array();
 ...
 function update_category($cid){
		...
		if($this->db->update('cste_category',$userdata)){
 ...
 function remove_category($cid){
	 ...
	 $this->db->where('cid',$cid);
	 if($this->db->delete('cste_category')){
		 return true;
	 }else{
 ...
 function insert_category(){
		...
		if($this->db->insert('cste_category',$userdata)){
...
// level function start
 ...
 function update_level($lid){
		...
		if($this->db->update('cste_level',$userdata)){
 ...
 function insert_level(){
		...
		if($this->db->insert('cste_level',$userdata)){
...
$question_type=$this->lang->line('long_answer');	
	...
	if($this->db->insert('cste_qbank',$insert_data)){
		$qid=$this->db->insert_id();
		$optionkeycounter = 4;
		if($ques_type=="0" || $ques_type==""){
		for($i=1;$i<=10;$i++){
			if($singlequestion[$optionkeycounter] != ""){
				...
				$this->db->insert("cste_options",$insert_options);
				$optionkeycounter++;
				}
