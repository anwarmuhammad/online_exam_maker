...
Class Quiz_model extends CI_Model
{
 ...
 function insert_quiz(){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->insert('cste_quiz',$userdata);
 ...
 function update_quiz($quid){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->update('cste_quiz',$userdata);
	  
	  $this->db->where('quid',$quid);
	  $query=$this->db->get('cste_quiz',$userdata);
	 $quiz=$query->row_array();
	 if($quiz['question_selection']=='1'){
		 
	  $this->db->where('quid',$quid);
	  $this->db->delete('cste_qcl');
	 ...
	 foreach($_POST['cid'] as $ck => $val){
		 ...
		 $this->db->insert('cste_qcl',$userdata);
		 }
		 }
	 }
		 $userdata=array(
...
/*
	 if($cid!='0'){
		 $this->db->where('cste_qbank.cid',$cid);
	 }
	 if($lid!='0'){
		 $this->db->where('cste_qbank.lid',$lid);
	 }
*/
	  
	 $query=$this->db->query("select * from cste_qbank join cste_category on cste_category.cid=cste_qbank.cid join cste_level on cste_level.lid=cste_qbank.lid 
	 where cste_qbank.qid in ($qids) order by FIELD(cste_qbank.qid,$qids) 
	 ");
	 return $query->result_array();
 ...
 function get_options($qids){
	 ...
	 $query=$this->db->query("select * from cste_options where qid in ($qids) order by FIELD(cste_options.qid,$qids)");
	 return $query->result_array();
...
$noq=$quiz['noq'];	
	...
	$wqids=implode(',',$qids);
	$noq=array();
	$query=$this->db->query("select * from cste_qbank join cste_category on cste_category.cid=cste_qbank.cid where qid in ($wqids) ORDER BY FIELD(qid,$wqids)  ");	
	...
	foreach($qcl as $k => $val){
		...
		$i=0;
	$query=$this->db->query("select * from cste_qbank join cste_category on cste_category.cid=cste_qbank.cid where cste_qbank.cid='$cid' and lid='$lid' ORDER BY RAND() limit $noq ");	
	$questions=$query->result_array();
	foreach($questions as $qk => $question){
		$qids[]=$question['qid'];
		if(!in_array($question['category_name'],$categories)){
	...
	$zeros=array();
	 ...
	 $this->db->insert('cste_result',$userdata);
 ...
 function open_result($quid,$uid){
	 $result_open=$this->lang->line('open');
		$query=$this->db->query("select * from cste_result  where cste_result.result_status='$result_open'  and cste_result.uid='$uid'  "); 
	if($query->num_rows() >= '1'){
		$result=$query->row_array();
return $result['rid'];		
	}else{
 ...
 function quiz_result($rid){
	...
	$query=$this->db->query("select * from cste_result join cste_quiz on cste_result.quid=cste_quiz.quid where cste_result.rid='$rid' "); 
	return $query->row_array(); 
...
function saved_answers($rid){
	...
	$query=$this->db->query("select * from cste_answers  where cste_answers.rid='$rid' "); 
	return $query->result_array(); 
 ...
 function assign_score($rid,$qno,$score){
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 $this->db->update('cste_result',$userdata);
	 ...
	 $qp=$r_qids[$qno];
		...
		if($$qp_score=='1'){
		  ...
		  $query_qp="update cste_qbank set  $crin  where qid='$qp'  ";
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 foreach($qids_perf as $qp => $qpval){
		 ...
		 if($qpval=='0'){
		  ...
		  $query_qp="update cste_qbank set no_time_served=(no_time_served +1)  $crin  where qid='$qp'  ";
	 $this->db->query($query_qp);
...
if($this->config->item('allow_result_email')){
	...
	$query = $this -> db -> query("select cste_result.*,cste_users.*,cste_quiz.* from cste_result, cste_users, cste_quiz where cste_users.uid=cste_result.uid and cste_quiz.quid=cste_result.quid and cste_result.rid='$rid'");
	$qrr=$query->row_array();
	...
	$this->db->delete('cste_answers');
	 ...
	 foreach($_POST['answer'] as $ak => $answer){
		 ...
		 if($_POST['question_type'][$ak] == '1' || $_POST['question_type'][$ak] == '2'){
			 ...
			 $qid=$qids[$ak];
			 $query=$this->db->query(" select * from cste_options where qid='$qid' ");
			 ...
			 $marks=0;
				foreach($answer as $sk => $ansval){
					...
					$this->db->insert('cste_answers',$userdata);
				$attempted=1;	
				}
				if($attempted==1){
					if($marks >= '0.99' ){
