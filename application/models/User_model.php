...
Class User_model extends CI_Model
{
 function login($username, $password)
 {
   ...
   $this -> db -> where('cste_users.password', MD5($password));
   }
    $this -> db -> where('cste_users.email', $username);
    $this -> db -> where('cste_users.verify_code', '0');
    $this -> db -> join('cste_group', 'cste_users.gid=cste_group.gid');
  $this->db->limit(1);
    $query = $this -> db -> get('cste_users');
 ...
 function user_list($limit){
	 if($this->input->post('search')){
		...
		$this->db->order_by('cste_users.uid','desc');
		 $this -> db -> join('cste_group', 'cste_users.gid=cste_group.gid');
		 $query=$this->db->get('cste_users');
 ...
 function verify_code($vcode){
	...
	$query=$this->db->get('cste_users');
		if($query->num_rows()=='1'){
			...
			$this->db->update('cste_users',$userdata);
			return true;
		}else{
 ...
 function insert_user(){
		...
		if($this->db->insert('cste_users',$userdata)){
		...
		$veri_code=rand('1111','9999');
		 if($this->config->item('verify_email')){
			$userdata['verify_code']=$veri_code;
				...
				$rresult=$this->db->update('cste_users',$userdata);
				if($this->session->userdata('logged_in_raw')){
				$this->session->unset_userdata('logged_in_raw');	
				}		
				}else{
				$rresult=$this->db->insert('cste_users',$userdata);
				}
		if($rresult){
			 if($this->config->item('verify_email')){
...
$new_password=rand('1111','9999');
 ...
 if($this->config->item('protocol')=="smtp"){
			...
			$this->db->where('email', $toemail);
 			$this->db->update('cste_users',$user_detail);
			return true;
			}
 ...
 function update_user($uid){
	 $logged_in=$this->session->userdata('logged_in');
		...
		if($this->input->post('password')!=""){
		 ...
		 $this->db->where('uid',$uid);
		if($this->db->update('cste_users',$userdata)){
 ...
 function update_group($gid){
		...
		if($this->db->update('cste_group',$userdata)){
 ...
 function get_user($uid){
	...
	$this->db->where('cste_users.uid',$uid);
	   $this -> db -> join('cste_group', 'cste_users.gid=cste_group.gid');
$query=$this->db->get('cste_users');
	 return $query->row_array();
...
//		'price'=>$this->input->post('price'),
		...
		if($this->db->insert('cste_group',$userdata)){
